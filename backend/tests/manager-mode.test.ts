...
async function setup(active = false) { const room = source(), repo = new MemoryManagerRepository(), service = new ManagerModeService(repo, async () => room); const state = await service.create(room.id, 'user0', { name: 'League', csv }); const act = (user: string, action: Parameters<typeof service.mutate>[3], id = randomUUID()) => service.mutate(state.id, user, id, action); if (active) {
    for (let i = 1; i < 8; i++)
        await act('user' + i, { type: 'INVITATION', accept: true });
    await act('user0', { type: 'GENERATE_FIXTURES' });
} return { room, repo, service, state, act }; }
it('imports exact squads and 70 free agents; snapshots independently and creates once', async () => { const f = await setup(); expect(f.state.players.filter(p => p.currentTeamId)).toHaveLength(192); expect(f.state.players.filter(p => !p.currentTeamId)).toHaveLength(70); expect(f.state.teams.every(t => t.transferBudgetUnits === 200)).toBe(true); expect((await f.service.create(f.room.id, 'user0', { name: 'Another' })).id).toBe(f.state.id); f.room.players[0]!.name = 'Changed'; expect((await f.service.state(f.state.id, 'user0')).players[0]!.name).toBe('Player 0'); expect(f.room.sequence).toBe(50); });
...
it('integrates authenticated REST, websocket invitations and exact reconnect state', async () => { const room = source(), rooms = new MemoryRoomRepository(); await rooms.create(room); const { app } = await buildApp({ authService: new StaticTokenAuthService(new Map(room.teams.map(t => [t.userId, { userId: t.userId }]))), repository: rooms, managerRepository: new MemoryManagerRepository(), logger: false, timersEnabled: false }); try {
    ...
    const created = await rest('user0', '/from-auction/' + room.id, { name: 'Realtime League', csv });
    expect(created.statusCode).toBe(200);
    const id = created.json().id;
    expect((await invited).payload).toEqual({ tournamentId: id });
    let waiting = receive(inbox, 'MANAGER_MODE_STATE');
    inbox.send(JSON.stringify({ type: 'SUBSCRIBE_MANAGER_MODE', payload: { tournamentId: id } }));
    expect((await waiting).payload).toMatchObject({ myTeamId: 'team1' });
    waiting = receive(inbox, 'MANAGER_MODE_STATE');
    ...
    expect(joined.payload).toMatchObject({ sequence: 2 });
    inbox.terminate();
    ...
    expect((await waiting).payload).toEqual(joined.payload);
...
it('completes after all results, permits corrections on reopen, and archives read-only', async () => {
 ...
 const finished=await f.act('user0',{type:'STATUS',status:'COMPLETED'});
 expect(finished.standings.every(row=>row.played===7)).toBe(true);
 await expect(f.act('user0',{type:'SCORE',fixtureId:state.fixtures[0]!.id,homeScore:2,awayScore:2})).rejects.toMatchObject({code:'INVALID_STATE'});
 await f.act('user0',{type:'STATUS',status:'ACTIVE'});
 await f.act('user0',{type:'STATUS',status:'ARCHIVED'});
 await expect(f.act('user0',{type:'WINDOW',open:true})).rejects.toMatchObject({code:'TOURNAMENT_ARCHIVED'});
});

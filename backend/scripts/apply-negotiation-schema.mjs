...
try{
 const tables=['manager_transfer_rules','manager_player_negotiation_profiles','manager_negotiation_sessions','manager_free_agent_offers','manager_negotiation_messages'];
 ...
 if(!found.length){const file=resolve('../supabase/migrations/'+version+'_manager_negotiation_engine.sql'),wrapped=resolve('scripts/negotiation-schema-apply.sql');try{writeFileSync(wrapped,'do $migration$ begin execute $schema$'+readFileSync(file,'utf8')+'$schema$; end; $migration$;');run(['db','query','--db-url',url,'--file',wrapped]);}finally{try{unlinkSync(wrapped);}catch{}}}
 run(['migration','repair',version,'--status','applied','--db-url',url,'--workdir',resolve('..')]);
 console.log('Negotiation schema verified; only migration '+version+' recorded as applied.');

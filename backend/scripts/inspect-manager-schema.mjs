...
try {
 const indexes=await db`select tablename,indexname,indexdef from pg_indexes where schemaname='public' and tablename like 'manager_%'`;
 ...
 await import('node:fs/promises').then(fs=>fs.writeFile('scripts/manager-schema-inspection.json',JSON.stringify({indexes,constraints,functions,columns,counts},null,2)));

...
const envSchema = z.object({
  ...
  // ---- Storage
  STORAGE:  z.enum(['memory', 'file', 'postgres']).default('file'),
  DATA_DIR: z.string().default('./data'),

  // ---- Authentication
  AUTH_MODE:      z.enum(['jwt', 'development', 'supabase']).default('jwt'),
...
export function readEnvironment(input: NodeJS.ProcessEnv = process.env) {
  ...
  if (env.NODE_ENV === 'production' && env.STORAGE !== 'postgres' && !env.PLAYER_CATALOG_PATH)

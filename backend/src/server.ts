...
const authService = env.AUTH_MODE === 'development'
...
const { app } = await buildApp({
  authService, origins: env.origins, logLevel: env.LOG_LEVEL,
  repository: env.STORAGE === 'file' ? new FileRoomRepository(env.DATA_DIR) : new MemoryRoomRepository(),
  playerRepository: env.PLAYER_CATALOG_PATH ? await CatalogPlayerRepository.fromFile(env.PLAYER_CATALOG_PATH) : new CatalogPlayerRepository(demoPlayers),
});

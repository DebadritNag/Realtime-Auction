...
export async function persistNegotiations(db:Db,t:Tournament,old?:Tournament){
 ...
 const changedProfiles=data.profiles.filter(p=>before?.profiles.some(old=>old.playerId===p.playerId&&['marketValueUnits','minimumValueUnits','preferredValueUnits','idealValueUnits'].some(key=>old[key as keyof NegotiationProfile]!==p[key as keyof NegotiationProfile])));
 if(changedProfiles.length)await db`update public.manager_player_negotiation_profiles p set market_value_units=v.market_value_units,minimum_value_units=v.minimum_value_units,preferred_value_units=v.preferred_value_units,ideal_value_units=v.ideal_value_units from jsonb_to_recordset(${db.json(changedProfiles.map(p=>({player_id:p.playerId,market_value_units:p.marketValueUnits,minimum_value_units:p.minimumValueUnits,preferred_value_units:p.preferredValueUnits,ideal_value_units:p.idealValueUnits})))}) as v(player_id uuid,market_value_units bigint,minimum_value_units bigint,preferred_value_units bigint,ideal_value_units bigint) where p.player_id=v.player_id and p.tournament_id=${t.id}`;

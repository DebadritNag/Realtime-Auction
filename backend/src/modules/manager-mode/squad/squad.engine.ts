...
export function squadAction(t:Tournament,user:string,a:SquadAction,now=Date.now()){
 ...
 const roles=formations[a.formation].flat();requireThat(a.slots.length===roles.length,'INVALID_LINEUP','Use exactly eleven formation slots.');
 a.slots.forEach((id,i)=>{if(!id)return;const p=owned.find(p=>p.id===id)!;requireThat((roles[i]==='GK')===(p.position==='GK'),'INVALID_POSITION','Goalkeepers must use the goalkeeper slot; outfield players must use outfield slots.');});

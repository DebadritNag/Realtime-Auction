...
import type {Tournament,ManagerPlayer,TournamentSummary} from './manager.types.js';
...
export class PostgresManagerTournamentRepository implements ManagerTournamentRepository{
 ...
 try{const rows=await this.db<{id:string;name:string;source_auction_id:string;code:string;status:TournamentSummary['status'];team_name:string;invitation:string;unread:string;sequence:string}[]>`select t.id,t.name,t.source_auction_id,a.code,t.status,team.team_name,m.status as invitation,
 (select count(*) from public.manager_notifications n where n.tournament_id=t.id and n.user_id=${user} and n.read_at is null) as unread,
 coalesce((select max((e.metadata->>'sequence')::bigint) from public.manager_tournament_events e where e.tournament_id=t.id),1) as sequence
 from public.manager_tournaments t join public.manager_tournament_members m on m.tournament_id=t.id and m.user_id=${user}
 join public.manager_tournament_teams team on team.id=m.team_id join public.auction_rooms a on a.id=t.source_auction_id order by t.updated_at desc`;
 return rows.map(r=>({id:r.id,name:r.name,sourceAuctionId:r.source_auction_id,sourceAuctionCode:r.code,status:r.status,teamName:r.team_name,invitation:r.invitation==='INVITED'?'PENDING':r.invitation as TournamentSummary['invitation'],unread:Number(r.unread),sequence:Number(r.sequence)}));
 ...
 notifications:notifications.map(n=>({id:n.id,userId:n.user_id,type:String((n.metadata as Record<string,unknown>|null)?.kind??n.type),title:n.title,message:n.message,read:n.read_at!==null,createdAt:ms(n.created_at),metadata:(n.metadata??{}) as Record<string,string>})),

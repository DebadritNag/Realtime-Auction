...
export class PostgresManagerTournamentRepository implements ManagerTournamentRepository{
 ...
 const [t]=await db<Row<'manager_tournaments'>[]>`select * from public.manager_tournaments where id=${id}`;if(!t)return null;
 ...
 db<Row<'manager_fixtures'>[]>`select * from public.manager_fixtures where tournament_id=${id} order by matchday,id`,
 db<Row<'manager_trade_offers'>[]>`select * from public.manager_trade_offers where tournament_id=${id} order by created_at,id`,
 db<Row<'manager_transfer_transactions'>[]>`select * from public.manager_transfer_transactions where tournament_id=${id} order by created_at,id`,
 ...
 transactions:transactions.map(r=>({id:r.id,playerId:r.player_id,fromTeamId:r.from_team_id,toTeamId:r.to_team_id!,type:r.type==='AUCTION_IMPORT'?'AUCTION_PURCHASE':r.type as Tournament['transactions'][number]['type'],amountUnits:Number(r.amount_units??0),tradeId:r.related_trade_id,at:ms(r.created_at)})),
 ...
 const added=draft.fixtures.filter(f=>!before.fixtures.some(x=>x.id===f.id));if(added.length)await tx`insert into public.manager_fixtures ${tx(added.map(f=>({id:f.id,tournament_id:id,matchday:f.matchday,home_team_id:f.homeTeamId,away_team_id:f.awayTeamId,status:f.status})))}`;

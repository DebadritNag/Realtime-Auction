...
/**
 * All participating team managers count toward the skip vote threshold.
 * This includes the host — they are a manager too, not an observer.
 * Required = total number of teams (unique manager user IDs).
 */
export function skipVoteState(room: Room, userId?: string) {
  const eligible = room.teams.map(t => t.userId);
...
export function updateSkipVote(room: Room, userId: string, playerId: string, activationId: string, remove: boolean, now: number) {
  ...
  // All teams are eligible — host included
  const eligible = room.teams.map(t => t.userId);

...
export function roomState(room: Room, userId: string, serverTime: number, connectedUsers: string[] = []) {
  ...
  return {
    ...
    teams: room.teams.map(t => publicTeam(t, room.settings)),
    currentUserTeam: publicTeam(team, room.settings), connectedUsers,

...
export function ManagerSetup({ auctionId }: {
    ...
    useEffect(() => { let live = true;setLoading(true);setError('');setPreview(null);managerService.preview(auctionId).then(p => { if (!live)
        return; setPreview(p); setName(p.name + ' League'); if (p.existingId)
        router.replace('/manager-mode/' + p.existingId); }).catch(e => { if (live)
        setError(e.message); }).finally(()=>{if(live)setLoading(false);}); return () => { live = false; }; }, [auctionId, router,retry]);
